...
<div class="row-fluid sortable">		
	<div class="box span12">
		<div class="box-header well" data-original-title>
			<h2><i class="icon-user"></i>&nbsp;<?php echo __('Evaluation States'); ?></h2>
		</div>
		<div class="box-content">
			<table class="table table-striped table-bordered">
				...
				<tbody>
					<?php foreach ($evaluationStates as $evaluationState): ?>
					<tr>
						<td><?php echo h($evaluationState['EvaluationState']['id']); ?>&nbsp;</td>
						<td><?php echo h($evaluationState['EvaluationState']['name']); ?>&nbsp;</td>
						<td><?php echo h($evaluationState['EvaluationState']['created']); ?>&nbsp;</td>
						<td><?php echo h($evaluationState['EvaluationState']['modified']); ?>&nbsp;</td>
						<td class="actions center">
							<?php echo $this->Charisma->iconButton(__('View'), 
								array('action' => 'view', $evaluationState['EvaluationState']['id']), 
								'icon-zoom-in icon-white', 'btn btn-small btn-success'); ?>
							<?php echo $this->Charisma->iconButton(__('Edit'),
								array('action' => 'edit', $evaluationState['EvaluationState']['id']), 
								'icon-edit icon-white', 'btn btn-small btn-info'); ?>
							<?php echo $this->Charisma->deleteButton(__('Delete'), 
								array('action' => 'delete', $evaluationState['EvaluationState']['id']), 
...
<li><?php echo $this->Charisma->iconLink(__('New Evaluation State'), 
		array('action' => 'add'), 
		'icon-plus'); ?>
</li>
<!-- Related -->
